        ...
        <div class="col-md-12">
            <div class="card ">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form class="forms-sample" action="{{ route('admin.template-jurnal.update', $template->id) }}" method="POST">
                        @csrf
                        @method('put')
                        <div class="row">
                            <div class="col-lg-4 col-md-6 col-sm-6 ">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="nama_template" id="nama_template" value="{{ old('nama_template') ?? $template->nama_template }}" class="form-control">
                                </div>
                            </div>
                            <div class="col-lg-8 col-md-6 col-sm-6 ">
                                <div class="form-group">
                                    <label for="keterangan">{{ __('Keterangan') }} <span class="text-danger">*</span> </label>
                                    <input type="text" name="keterangan" id="keterangan" value="{{ old('keterangan') ?? $template->keterangan }}" class="form-control">
                                </div>
                            </div>
                            ...
                            <div class="col-md-2 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <select name="per_tanggal" id="per_tanggal" class="form-control">
                                        @for($i = 1; $i < 31; $i++)
                                            <option value="{{ $i }}" {{ old('per_tanggal') ?? $template->per_tanggal == $i ? 'selected' : '' }}>{{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-9 col-md-8">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="uraian" id="uraian" value="{{ old('uraian') ?? $template->uraian }}" class="form-control">
                                </div>
                            </div>
                        </div>
                        <div class="row">
...
@push('script')
    ...
    <script>
        ...
        function field_dinamis() {
            ...
            let html = `
                <tr class="rowComponent">
                    <input type="hidden" width="10px" name="jurnals[${index}][id]" class="id" value="${id}">
                    <td class="no" hidden>
                    ...
                    <td>
                        <input type="text" name="jurnals[${index}][debit]" class="form-control debit" value="${debit}"
                            placeholder="0" onkeypress="onlyNumber(event)" oninput="jumlahin()" autocomplete="off">
                    </td>
                    <td>
                        <input type="text" name="jurnals[${index}][kredit]" class="form-control kredit" value="${kredit}"
                            placeholder="0" onkeypress="onlyNumber(event)" oninput="jumlahin()" autocomplete="off">
                    </td>
            `
            if (index > 0) {
        ...
        $.ajax({
            type: 'get',
            url: '{{ route('api.template-jurnal.selected', ':id') }}'.replace(':id', '{{ $template->id }}'),
            dataType: 'json',
            success: results => {
                const data = results.data;
                const template_details = results.data.template_details;

                template_details.forEach((detail, index) => {
                    field_dinamis(detail.id, detail.akun_id, detail.debit, detail.kredit);
                    if ((index + 1) == template_details.length) {
        ...
        $(document).ready(function() {
            ...
            $('#sumber').val('{{ old('sumber') ?? $template->sumber }}').trigger('change');
            ...
            $('#frekuensi').val('{{ old('frekuensi') ?? $template->frekuensi }}').trigger('change');
            ...
            $('#per_tanggal').val('{{ old('per_tanggal') ?? $template->per_tanggal }}').trigger('change');

            $("#kontak_id").select2({
                placeholder: "-- Pilih Kontak --",
                ajax: {
                    url: '{{ route('api.select2.get-kontak-paginate') }}',
                    type: 'post',
                    dataType: 'json',
                    data: params => {
                        return {
            ...
            let url_kontak = '{{ route('api.select2.get-kontak.selected', ':id') }}'.replace(':id', '{{ $template->kontak_id }}')
            $.ajax({
                type: 'get',
                url: url_kontak
            }).then((data) => {
        ...
        let url_divisi = '{{ route('api.select2.get-divisi.selected', ':id') }}'.replace(':id', '{{ $template->divisi_id }}')
        $.ajax({
            type: 'get',
            url: url_divisi
        }).then((data) => {
